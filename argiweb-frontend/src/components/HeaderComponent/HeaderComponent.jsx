...
const HeaderComponent = () => {
  ...
  if (token) {
    ...
    const encodedName = decodedToken.payload.name
      ? utf8Decoder.decode(
          new Uint8Array(
            [...decodedToken.payload.name].map((c) => c.charCodeAt(0))
          )
        )
      : null;
  ...
  const menu = (
    <Menu>
      <Menu.Item key="1">
        <span onClick={() => navigate(isAdmin ? "/system" : "/profile-user")}>
          {label}
        </span>
      </Menu.Item>
      <Menu.Item key="2">
  ...
  return (
    <>
      <div className="announcement-header">
        MIỄN PHÍ GIAO HÀNG CHO ĐƠN HÀNG TỪ 499K
      </div>
